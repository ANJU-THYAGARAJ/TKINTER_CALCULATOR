...
i=0
def get_variable(num):#getting the value to text file
    global i
...
def calculate():#calculating
    ...
    try:
        ...
        clear_all()
        display.insert(0,"error")
Button(root,text="1",command=lambda:get_variable(1)).grid(row=2,column=0)#adding button
...
Button(root,text="(",command=lambda:get_op('(')).grid(row=2,column=4)
Button(root,text=")",command=lambda:get_op(')')).grid(row=3,column=4)
Button(root,text="pi",command=lambda:get_op('*3.14')).grid(row=4,column=4)
Button(root,text="%",command=lambda:get_op('%')).grid(row=5,column=4)
Button(root,text="exp",command=lambda:get_op('**')).grid(row=2,column=5)
Button(root,text="x!",command=lambda:get_op('!')).grid(row=3,column=5)
Button(root,text="^",command=lambda:get_op(**2)).grid(row=4,column=5)
